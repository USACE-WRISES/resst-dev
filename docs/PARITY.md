...
| 28 | The interactive map is Leaflet (DOM elements + image tiles), not MapLibre GL (WebGL): the Esri basemap is Esri's raster World Topographic Map tiles rather than the vector style (no hillshade composite, no basemap failure/retry path); site labels are placed by a greedy first-come pass (up to 150 in view) instead of a collision engine; box select picks sites by marker centre within 6.5 px of the box, clamped to the visible map, against the current filtered set; and the national inventory layer is a canvas redrawn once per settle (its dots freeze, scaled, during a zoom animation) rather than a continuously animated WebGL layer. MapLibre remains only for the Dam Report's static map figure and the `?diag=1` benchmark | USACE workstations run RESST through DoD remote browser isolation, which streams every WebGL canvas at ~4.6 fps while DOM content animates locally; the owner chose one engine (Leaflet) over maintaining two (decision 2026-09-02, `notes/2026-09-02-usace-map-lag-isolation-and-plan.md`) |
