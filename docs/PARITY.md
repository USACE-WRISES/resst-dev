...
| 27 | Round-3 polish (owner's list, 2026-08-30): every Selected Data card starts collapsed (sticky per session); the panel is drag/keyboard resizable (280–620 px, persisted); prose across the UI and Help drops em dashes and long disclaimers, with the bolded research-use note moved onto the welcome splash; the Evidence section spells out RESSED survey codes with an honest glossary, full dates, agencies, and direct links to the scanned original datasheets (`reservoir_id` < 100000 = the RESIS dsnum, pattern verified); downstream network dams draw larger; a Drainage area toggle delineates the upstream basin live from USGS NLDI (the app's one on-demand external data API); and a Report button compiles the whole panel into a downloadable/printable dossier for either target kind | Owner's seven-point round-3 request; NLDI and the report's map figure both fail soft (status line / placeholder) so the static app never depends on the external service being up |
| 28 | The interactive map is moving from MapLibre (WebGL) to Leaflet (DOM + image tiles) in three phases; during the transition machines whose WebGL is software-rendered get the Leaflet map automatically (`?map=leaflet` / `?map=maplibre` and `localStorage resst.mapEngine` force either). On Leaflet: the Esri basemap is Esri's raster World Topographic Map tiles rather than the vector style; site labels are placed by a greedy first-come pass (up to 150 in view) instead of MapLibre's collision engine; box select picks sites by marker centre within 6.5 px of the box, clamped to the visible map, against the current filtered set (the same rule now applies on MapLibre, replacing the rendered-feature query); zoom levels sit one step higher than MapLibre's for the same scale (256 px tiles); and the national inventory layer is a canvas redrawn once per settle (its dots freeze, scaled, during a zoom animation) rather than a continuously animated WebGL layer | USACE workstations run RESST through DoD remote browser isolation, which streams every WebGL canvas at ~4.6 fps while DOM content animates locally; the owner chose one engine (Leaflet) over maintaining two (decision 2026-09-02, `notes/2026-09-02-usace-map-lag-isolation-and-plan.md`) |
