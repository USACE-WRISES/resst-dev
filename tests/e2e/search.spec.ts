...
test("keyboard spans groups; Enter on a place flies the map and drops a pin", async ({ page }) => {
  ...
  await page.waitForFunction(
    () => {
      ...
      if (!info || info.isMoving()) return false;
      const c = info.getCenter();
  ...
  await expect(page.locator(".leaflet-popup").getByText("Platte River")).toBeVisible();
...
test("choosing a site still selects it and clears any place pin", async ({ page }) => {
  ...
  await expect(page.locator(".dom-place-marker")).toHaveCount(0); // pin cleared
});
